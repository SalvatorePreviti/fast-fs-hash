...
/**
 * Multi-mutation state.
 * Three groups of files for realistic change-distribution benchmarks:
 *   sizeChanged:    content rewritten with different size (stat+size differ → needs hash)
 *   contentChanged: content rewritten with same size (stat differs, size same → needs hash, content differs)
 *   metadataOnly:   mtime bumped, content unchanged (stat differs, size same → content same)
 *
 * Each entry: { path: string, origSize: number }
 */
let _multiMut = { sizeChanged: [], contentChanged: [], metadataOnly: [] };
let _multiMutCounter = 0;
...
function generate() {
  ...
  // Select ~150 files for multi-mutation benchmarks.
  // Pick every 5th file, then split into 3 groups of ~50.
  // Skip empty files (size 0) — can't meaningfully mutate content.
  _multiMutCounter = 0;
  const candidates = [];
  for (let i = 0; i < allPaths.length; i += 5) {
    ...
    try {
      ...
      if (sz > 0) {
        candidates.push({ path: p, origSize: sz });
      }
    } catch {
      // skip files that can't be stat'd
    }
  }
  const third = Math.floor(candidates.length / 3);
  _multiMut = {
    sizeChanged: candidates.slice(0, third),
    contentChanged: candidates.slice(third, third * 2),
    metadataOnly: candidates.slice(third * 2),
  };
...
/**
 * Mutate ~150 files with a realistic distribution of changes.
 * Three groups (~50 each):
 *   sizeChanged:    rewrite with different size (stat+size differ → needs hash)
 *   contentChanged: rewrite with same size, different content (stat differs, content differs)
 *   metadataOnly:   bump mtime only (stat differs, content unchanged)
 *
 * Uses incrementing counter for deterministic, unique mutations per call.
 */
function mutateMultipleFiles() {
  _multiMutCounter++;
  const baseTime = 1700000000000 + _multiMutCounter * 1000;

  // Group 1: content changed, different size (append 1-8 bytes)
  for (let i = 0; i < _multiMut.sizeChanged.length; i++) {
    const { path: p, origSize } = _multiMut.sizeChanged[i];
    const extra = 1 + ((_multiMutCounter + i) % 8);
    const buf = Buffer.allocUnsafe(origSize + extra);
    // Fill with deterministic content based on counter + index
    const rng = xorshift32((_multiMutCounter * 1000 + i) | 1);
    for (let j = 0; j + 3 < buf.length; j += 4) {
      buf.writeUInt32LE(rng(), j);
    }
    fs.writeFileSync(p, buf);
    const t = new Date(baseTime + i);
    fs.utimesSync(p, t, t);
  }

  // Group 2: content changed, same size (flip bytes in middle)
  for (let i = 0; i < _multiMut.contentChanged.length; i++) {
    const { path: p, origSize } = _multiMut.contentChanged[i];
    const buf = Buffer.allocUnsafe(origSize);
    const rng = xorshift32((_multiMutCounter * 2000 + i) | 1);
    for (let j = 0; j + 3 < buf.length; j += 4) {
      buf.writeUInt32LE(rng(), j);
    }
    fs.writeFileSync(p, buf);
    const t = new Date(baseTime + 1000 + i);
    fs.utimesSync(p, t, t);
  }

  // Group 3: metadata only (touch mtime, don't change content)
  for (let i = 0; i < _multiMut.metadataOnly.length; i++) {
    const { path: p } = _multiMut.metadataOnly[i];
    const t = new Date(baseTime + 2000 + i);
    fs.utimesSync(p, t, t);
  }
}
...
if (require.main === module) {
  ...
  module.exports = { generate, mutateModFile, mutateMultipleFiles };
}
