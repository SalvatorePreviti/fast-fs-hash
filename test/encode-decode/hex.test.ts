...
describe("hashToHex", () => {
  ...
  it("returns empty string on invalid offset or input", () => {
    const hash = new Uint8Array(16);
    expect(hashToHex(hash, 1)).toBe("");
    expect(hashToHex(hash, -1)).toBe("");
    expect(hashToHex(null as unknown as Uint8Array)).toBe("");
    expect(hashToHex(undefined as unknown as Uint8Array)).toBe("");
  });
});

describe("hashesToHexArray", () => {
  it("returns empty array on invalid length", () => {
    expect(hashesToHexArray(new Uint8Array(1))).toEqual([]);
    expect(hashesToHexArray(new Uint8Array(17))).toEqual([]);
    expect(hashesToHexArray(null as unknown as Uint8Array)).toEqual([]);
    expect(hashesToHexArray(undefined as unknown as Uint8Array)).toEqual([]);
  });
