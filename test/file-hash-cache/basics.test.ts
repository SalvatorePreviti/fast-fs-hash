...
import { FileHashCache, KeyedLock } from "fast-fs-hash";
...
describe("FileHashCache [native]", () => {
  ...
  describe("check (no write)", () => {
    ...
    it("returns a valid status with empty file list and no cache", async () => {
      const ctx = await FileHashCache.open(cachePath("val-empty"), FIXTURE_DIR, [], 1);
  ...
  describe("options", () => {
    it("version is treated as u32", async () => {
      const ctx = await FileHashCache.open(cachePath("ver-u32"), FIXTURE_DIR, [], -1);
    ...
    it("different fingerprint per open", async () => {
      ...
      const ctx2 = await FileHashCache.open(cp, FIXTURE_DIR, files, 1, fp1);
      expect(ctx2.status).toBe("upToDate");

      // Read with fp2 — should be stale
      const ctx3 = await FileHashCache.open(cp, FIXTURE_DIR, files, 1, fp2);
  ...
  describe("reuse mode (files=null)", () => {
    it("reuses file list from old cache on disk", async () => {
      ...
      // Reuse: files=null
      const ctx2 = await FileHashCache.open(cp, FIXTURE_DIR, null, 1);
      expect(ctx2.status).toBe("upToDate");
    });
    ...
    it("returns missing when no old cache exists", async () => {
      ...
      const ctx = await FileHashCache.open(cp, FIXTURE_DIR, null, 1);
      expect(ctx.status).toBe("missing");
    });
    ...
    it("detects changes when files on disk have changed", async () => {
      ...
      const ctx3 = await FileHashCache.open(cp, FIXTURE_DIR, null, 1);
      expect(ctx3.status).toBe("upToDate");
  ...
  describe("concurrency", () => {
    it("concurrent open+write calls on same path complete without error", async () => {
      const cp = cachePath("concurrent");
      const files = [fixtureFile("a.txt")];

      // Launch multiple locked open+write calls concurrently
      const doSync = async () => {
        await using _lock = await KeyedLock.acquire(cp);
        const ctx = await FileHashCache.open(cp, FIXTURE_DIR, files, 1);
      ...
      const results = await Promise.all([doSync(), doSync(), doSync()]);

      // First call creates, subsequent ones see upToDate
      const statuses = results.sort();
      expect(statuses.filter((s) => s === "upToDate").length).toBeGreaterThanOrEqual(2);
    });
  });
  ...
  describe("fingerprint update from write options", () => {
    it("updates fingerprint when write options specify one", async () => {
      ...
      // fp1 should be stale
      const ctx3 = await FileHashCache.open(cp, FIXTURE_DIR, files, 1, fp1);
      expect(ctx3.status).toBe("stale");
    });
  });
  ...
  describe("update file list preserves user data", () => {
    ...
    it("changing files + fingerprint writes new fingerprint", async () => {
      ...
      // Seed with fp1
      const ctx1 = await FileHashCache.open(cp, FIXTURE_DIR, files1, 1, fp1);
      await ctx1.write();

      // Change files and fingerprint in write
      const ctx2 = await FileHashCache.open(cp, FIXTURE_DIR, files1, 1, fp1);
      await ctx2.write({
        files: files2,
        rootPath: FIXTURE_DIR,
        fingerprint: fp2,
      ...
      const ctx3 = await FileHashCache.open(cp, FIXTURE_DIR, files2, 1, fp2);
      expect(ctx3.status).toBe("upToDate");

      // fp1 should be stale
      const ctx4 = await FileHashCache.open(cp, FIXTURE_DIR, files2, 1, fp1);
